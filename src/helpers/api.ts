...
import { IComment, ILike, IResponse, IUser, PartialUser } from "./types";
...
export const handleComments = async (id:number):Promise<IComment>=>{
  const response = await Axios.post('/posts/comment/' + id)
  return response.data
}

export const handleLikes = async(id:number):Promise<ILike>=>{
  const response = await Axios.post('/posts/react/' + id)
  return response.data
}

export const handleRemoveeLikes = async(id:number):Promise<ILike>=>{
  const response = await Axios.delete('/posts/react/' + id)
  return response.data
}